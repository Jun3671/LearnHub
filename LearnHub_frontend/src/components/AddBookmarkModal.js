...
function AddBookmarkModal({ isOpen, onClose, onSuccess }) {
  ...
  const fetchCategories = async () => {
    try {
      ...
      console.log('카테고리 API 응답:', response); // 디버깅용 로그
      console.log('카테고리 데이터:', response.data); // 디버깅용 로그
      // 응답이 배열인지 확인하고 설정
      if (Array.isArray(response.data)) {
        setCategories(response.data);
        console.log('카테고리 설정 완료:', response.data.length, '개'); // 디버깅용 로그
      } else {
        console.warn('카테고리 응답이 배열이 아닙니다:', response.data);
        setCategories([]);
      }
    } catch (err) {
      console.error('Failed to fetch categories:', err);
      console.error('에러 상세:', err.response); // 디버깅용 로그
    ...
    <div className="fixed inset-0 bg-neutral-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in">
      <div className="bg-white rounded-xl shadow-modal max-w-2xl w-full max-h-[90vh] overflow-y-auto animate-scale-in">
        ...
        <div className="sticky top-0 bg-white border-b border-neutral-200/60 px-6 py-4 flex items-center justify-between rounded-t-xl">
          <h2 className="text-xl font-semibold text-neutral-900">새 북마크 추가</h2>
          <button
            onClick={onClose}
